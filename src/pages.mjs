...
export function renderRulesComparisonPage(entries, footerEntry = null) {
  ...
  const englishCaptureAnnouncement = `A capture is announced with the capture square, calculated from the captured player’s side. ${enPassantCaptureAnnouncement} Neither the capturing man nor the captured man is named.`;
  ...
  const crazyKriegIllegalResponse = 'Illegal tries are public: the referee says “No” for hidden-position illegality, or “Impossible” / “Hell no” for impossible moves.';
  ...
  const comparisonRows = [
    ['Referee response to illegal tries', publicIllegalResponse, publicIllegalResponse, 'Illegal attempts are private: only the mover sees “Illegal move”, and the opponent hears nothing until a move is complete.', `${publicIllegalResponse} RAND also lets a player ask for the opponent’s last-move rebuff count.`, 'The umpire announces that a move is illegal; it must be retracted and the player must make a legal move.', crazyKriegIllegalResponse],
    ...
    ['Promotion announcements', silentPromotion, silentPromotion, silentPromotion, 'The fact that a pawn promotes is announced, but not the promoted piece type or promotion square.', 'No separate promotion announcement is listed; the umpire communicates only the details permitted by the rules.', 'Promotion is not announced. If the promoted pawn is later captured, it enters reserve and is announced as a pawn.'],
