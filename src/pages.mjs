...
export function renderRulesComparisonPage(entries, footerEntry = null) {
  const comparisonRows = [
    ['Referee response to illegal tries', 'Referee says “Illegal” or “No” for illegal moves on the true board, and “Nonsense” for impossible or repeated tries on your own board, which the player must already know are impossible.', 'Referee says “Illegal” or “No” for illegal moves on the true board, and “Nonsense” for impossible or repeated tries on your own board, which the player must already know are impossible.', 'Referee does not publicly announce “illegal move”; the opponent is told nothing until the move is complete.', 'Referee announces each rebuff, or “no”, during the mover’s attempts. Moves illegal per se get a special rebuff, for example “hell, no”; a player may demand a count of the opponent’s no’s from the last move.'],
    ['Capture announcements', 'Capture square (where the piece is removed from) is announced to both players after a legal capture.', 'Capture square (where the piece is removed from) is announced to both players after a legal capture, and the referee also says whether the captured material was a pawn or a piece.', 'Same capture information as Cincinnati, in fixed ICC wording: “Pawn at &lt;square&gt; captured” or “Piece at &lt;square&gt; captured”.', 'Announces that a capture happened, where it happened, and whether the captured man was a pawn or a piece. Square names are given with reference to the mover.'],
    ['Check announcements', 'File, rank, long diagonal, short diagonal, knight, and double checks are announced.', 'File, rank, long diagonal, short diagonal, knight, and double checks are announced.', 'File, rank, long diagonal, short diagonal, knight, and double checks are announced.', 'File, rank, long diagonal, short diagonal, knight, and multiple checks are announced. Pawn checks are announced as diagonal checks, without saying they are pawn checks.'],
    ['Pawn-capture handling — “Any?” rule handling', 'No built-in “Any?” rule in the original ruleset. A compatible modification is documented: the player may ask, and if the answer is “yes”, then must do a pawn capture.', 'Before each ply starts, the referee publicly announces that the player has a pawn capture whenever at least one legal pawn capture exists. The player may now try to make captures with their pawns, or may not.', 'Before each ply starts, the referee publicly announces the number of legal capturing pawn moves. The player may now try to make captures with their pawns, or may not.', 'Before moving, the referee announces the squares on which the mover’s pawns have currently valid capture tries. When in check, only pawn tries that would eliminate the check are announced.'],
    ['Promotion announcements', 'Promotion is not specifically announced; the player secretly indicates the promotion piece to the referee.', 'Promotion is not announced and should be handled silently.', 'Promotion is not announced as promotion; only normal capture and check announcements are made.', 'The fact that a pawn promotes is announced, but not the promoted piece type or promotion square.'],
