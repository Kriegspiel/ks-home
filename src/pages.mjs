...
export function renderRulesComparisonPage(entries, footerEntry = null) {
  ...
  const englishCaptureAnnouncement = `A capture is announced with the capture square, calculated from the captured player’s side. ${enPassantCaptureAnnouncement}`;
  ...
  const comparisonRows = [
    ['Referee response to illegal tries', publicIllegalResponse, publicIllegalResponse, 'Illegal attempts are private: only the mover sees “Illegal move”, and the opponent hears nothing until a move is complete.', publicIllegalResponse, publicIllegalResponse, publicIllegalResponse],
    ['Capture announcements', captureSquareOnly, captureSquareAndType, captureSquareAndType, captureSquareAndType, englishCaptureAnnouncement, crazyKriegCaptureAnnouncement],
    ['Check announcements', directionalChecks, directionalChecks, directionalChecks, directionalChecks, englishCheckAnnouncement, directionalChecks],
    ['Pawn-capture handling — “Any?” rule handling', 'No built-in “Any?” rule in the original ruleset. A compatible modification is documented: the player may ask, and if the answer is “yes”, then must do a pawn capture.', 'Before each ply starts, the referee publicly announces that the player has a pawn capture whenever at least one legal pawn capture exists. The player may now try to make captures with their pawns, or may not.', 'Before each ply starts, the referee publicly announces the number of legal capturing pawn moves. The player may now try to make captures with their pawns, or may not.', 'Before moving, the referee announces the squares on which the mover’s pawns have currently valid capture tries.', englishAnyQuestion, englishAnyQuestion],
    ['Promotion announcements', silentPromotion, silentPromotion, silentPromotion, 'The fact that a pawn promotes is announced, but not the promoted piece type or promotion square.', silentPromotion, 'Promotion is not announced. If the promoted pawn is later captured, it enters reserve and is announced as a pawn.'],
    ['Reserves and drops', noReservesOrDrops, noReservesOrDrops, noReservesOrDrops, noReservesOrDrops, noReservesOrDrops, crazyKriegReservesAndDrops],
  ...
  ].map(([label, berkeley, cincinnati, wild16, rand, english, crazykrieg]) => `<tr><th scope="row">${label}</th><td>${berkeley}</td><td>${cincinnati}</td><td>${wild16}</td><td>${rand}</td><td>${english}</td><td>${crazykrieg}</td></tr>`).join('');
