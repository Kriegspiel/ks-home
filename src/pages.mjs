...
export function renderHomePage({ rulesCount = 0, blogCount = 0, homeContent, footerEntry }) {
  ...
  return renderShell({
    ...
    activeNav: '/',
    canonicalPath: '/',
    main: `<section id="hero" class="hero-card"><h1>${esc(content.heroTitle)}</h1><p class="hero-card__lede">${esc(content.heroLede)}</p><div class="hero-card__actions"><a class="button-link button-link--primary" href="${esc(content.heroPrimaryCtaHref)}" data-telemetry-event="home_cta_click">${esc(content.heroPrimaryCtaLabel)}</a><a class="button-link button-link--secondary" href="${esc(content.heroSecondaryCtaHref)}">${esc(content.heroSecondaryCtaLabel)}</a></div><dl class="hero-card__stats"><div><dt>${esc(content.statsRulesLabel)}</dt><dd>${rulesCount}</dd></div><div><dt>${esc(content.statsUpdatesLabel)}</dt><dd>${blogCount}</dd></div><div><dt>${esc(content.statsThirdLabel)}</dt><dd>${esc(content.statsThirdValue)}</dd></div></dl></section><section id="how-it-works" class="content-section home-section"><div class="section-heading"><h2>${esc(content.flowTitle)}</h2><p>${esc(content.flowIntro)}</p></div><ol class="feature-grid home-list"><li class="home-list__item"><strong>${esc(content.flowStep1Title)}</strong><span>${esc(content.flowStep1Body)}</span></li><li class="home-list__item"><strong>${esc(content.flowStep2Title)}</strong><span>${esc(content.flowStep2Body)}</span></li><li class="home-list__item"><strong>${esc(content.flowStep3Title)}</strong><span>${esc(content.flowStep3Body)}</span></li></ol></section><section id="key-features" class="content-section home-section"><div class="section-heading"><h2>${esc(content.featuresTitle)}</h2><p>${esc(content.featuresIntro)}</p></div><div class="feature-grid feature-grid--three home-grid"><article class="home-grid__item"><h3>${esc(content.feature1Title)}</h3><p>${esc(content.feature1Body)}</p></article><article class="home-grid__item"><h3>${esc(content.feature2Title)}</h3><p>${esc(content.feature2Body)}</p></article><article class="home-grid__item"><h3>${esc(content.feature3Title)}</h3><p>${esc(content.feature3Body)}</p></article></div></section><section id="cta" class="content-section home-section"><div><div class="section-heading"><h2>${esc(content.ctaTitle)}</h2><p>${esc(content.ctaBody)}</p></div><div class="cta-panel__actions"><a class="button-link button-link--primary" href="${esc(content.ctaPrimaryHref)}">${esc(content.ctaPrimaryLabel)}</a><a class="button-link button-link--secondary" href="${esc(content.ctaSecondaryHref)}">${esc(content.ctaSecondaryLabel)}</a></div></div></section><section id="trust-snippet" class="content-section home-section"><div class="section-heading"><h2>${esc(content.trustTitle)}</h2></div><div class="feature-grid feature-grid--two home-grid"><article class="home-grid__item"><h3>${esc(content.trustRulesTitle)}</h3><p>${esc(interpolate(content.trustRulesBodyTemplate))}</p></article><article class="home-grid__item"><h3>${esc(content.trustUpdatesTitle)}</h3><p>${esc(interpolate(content.trustUpdatesBodyTemplate))}</p></article></div></section>`
  });
}

export function renderLeaderboardPage(entries = [], footerEntry = null) {
...
export function renderRulesPage(entries, changelogEntries, footerEntry = null) {
  ...
  const cards = primary.map((entry) => {
    const sections = sectionsFromBody(entry.body, 3).map((section) => `<li>${section}</li>`).join('');
    ...
    return `<article class="surface-card rules-tile"><p class="rules-tile__eyebrow">Ruleset</p><h2>${prettyRuleLabel(entry.metadata.slug)}</h2><p>${esc(entry.metadata.summary)}</p><ul class="rules-tile__meta"><li>Version: ${esc(entry.metadata.version || 'n/a')}</li><li>Revision: <code>${esc(entry.metadata.revision || 'n/a')}</code></li><li>Last reviewed: ${esc(entry.metadata.lastReviewedAt || entry.metadata.updatedAt || 'n/a')}</li></ul><h3>Highlights</h3><ul>${sections || '<li>No section headers parsed</li>'}</ul><div class="rules-tile__actions"><a class="button-link button-link--primary" href="/rules/${entry.metadata.slug}">Read ${prettyRuleLabel(entry.metadata.slug)} rules</a><span>${link}</span></div></article>`;
  }).join('');
  return renderShell({ footerEntry, title: 'Kriegspiel — Rules', description: 'Versioned rulesets with traceable revisions and changelog linkage.', activeNav: '/rules', canonicalPath: '/rules', structuredData: { '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Kriegspiel Rules', url: absUrl('/rules') }, main: `<section class="content-section"><div class="section-heading"><h1>Rules</h1><p>Choose a published ruleset, then use the comparison page when you need the differences at a glance.</p></div><div class="feature-grid feature-grid--two rules-grid">${cards}</div><aside class="cta-panel rules-comparison-callout"><div><h2>Need the differences first?</h2><p>See the overall comparison before picking a ruleset.</p></div><div class="cta-panel__actions"><a class="button-link button-link--secondary" href="/rules/comparison/">Open rules comparison</a></div></aside></section>` });
}

export function renderRuleDetailPage(entry, changelogEntries, footerEntry = null) {
  const changelog = changelogEntries.find((candidate) => candidate.metadata.slug === entry.metadata.changelogSlug);
  const sectionLinks = sectionsFromBody(entry.body, 6).map((section) => `<li>${section}</li>`).join('');
  return renderShell({ footerEntry, title: `Kriegspiel — ${entry.metadata.title}`, description: entry.metadata.summary, activeNav: '/rules', canonicalPath: `/rules/${entry.metadata.slug}`, structuredData: { '@context': 'https://schema.org', '@type': 'TechArticle', headline: entry.metadata.title, datePublished: entry.metadata.publishedAt, dateModified: entry.metadata.updatedAt, author: { '@type': 'Organization', name: entry.metadata.author }, mainEntityOfPage: absUrl(`/rules/${entry.metadata.slug}`) }, main: `<section class="content-section"><article class="prose-card"><p class="rules-tile__eyebrow">Ruleset</p><h1>${esc(entry.metadata.title)}</h1><p>${esc(entry.metadata.summary)}</p><div class="rule-detail-meta"><span>Version ${esc(entry.metadata.version || 'n/a')}</span><span>Revision <code>${esc(entry.metadata.revision || 'n/a')}</code></span><span>Last reviewed ${esc(entry.metadata.lastReviewedAt || entry.metadata.updatedAt || 'n/a')}</span></div><div class="rule-detail-actions"><a class="button-link button-link--secondary" href="/rules">Back to rules</a><a class="button-link button-link--secondary" href="/rules/comparison/">Compare rulesets</a>${changelog ? `<a class="button-link button-link--secondary" href="/changelog/${changelog.metadata.slug}">Linked changelog</a>` : ''}</div>${sectionLinks ? `<section class="content-section"><h2>On this page</h2><ul>${sectionLinks}</ul></section>` : ''}${entry.bodyHtml}</article></section>` });
}
