...
export function renderRulesComparisonPage(entries, footerEntry = null) {
  ...
  const captureSquareOnly = 'After a legal capture, the captured square is announced to both players. For en passant, that is the pawn’s removal square.';
  ...
  const englishCaptureAnnouncement = 'A capture is announced with the capture square, calculated from the captured player’s side. Neither the capturing man nor the captured man is named.';
  const englishCheckAnnouncement = 'Long, short, rank, file, and knight checks are announced.';
