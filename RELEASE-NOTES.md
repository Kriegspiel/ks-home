...
- **Subscription**: keep only Llama 4 Maverick and Gemma 4 31B visible in the
  public tier matrix while older variants stay available by direct app profile.
...
- **Rules**: soften the Dutch status label to “not playable online yet” and
  center status-pill text with more breathing room.
...
- **Levels**: split the pricing matrix into a frozen header pane and a
  separate scrollable body pane so the `Feature`/tier row itself is no longer
  part of the vertical scroll area.

## ks-home v. 1.3.26

- **Levels**: pin the pricing matrix `Feature`/tier header row directly so it
  remains frozen while scrolling the table.

## ks-home v. 1.3.25

- **Levels**: harden the pricing matrix frozen panes so the top row, first
  column, and corner cell remain pinned during table scrolling.
...
- **Homepage**: add a structural line break between the games count and the
  “games played” label in the stat tile.

## ks-home v. 1.3.12

- **Homepage**: stack the games-played tile so the count and “games played”
  label appear on separate lines.
